...
export function WishlistIcon(props) {
  const { productoId, className, removeCallback } = props;
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await wishlistCtrl.check(user.id, productoId);
  ...
  const addWishlist = async () => {
    const response = await wishlistCtrl.add(user.id, productoId);
    setHasWishlist(response);
  };
